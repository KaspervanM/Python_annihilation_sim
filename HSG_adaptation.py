...
f = SCALE * 8.987551787e9 * 1e-2  # Nm^2C^-2
...
def update_particle(b) -> None:
    ...
    self_ind = Atoms.index(b)
    for i, ball_elem in enumerate(Atoms):
        if i != self_ind:
...
def update():
    for ball_elem in Atoms:
        update_particle(ball_elem)
    for index, ball_elem2 in enumerate(Atoms):
        ball_elem2.velocity += ball_elem2.force / ball_elem2.mass
        print(Atoms.index(ball_elem2), ": ",
              ball_elem2.force, ": ", ball_elem2.velocity)
        ball_elem2.pos += ball_elem2.velocity * dt
        apos[index] += ball_elem2.velocity * dt
...
while True:
    ...
    # for i in range(Natoms):
    update()  # Atoms[i].pos = apos[i] = apos[i] + (p[i]/mass)*dt
